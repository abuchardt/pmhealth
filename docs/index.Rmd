...
The Rasch model for polytomous items[^2] [@Andersen1977]:

[^2]: Frequently referred to as the Rating Scale Model (RSM) or the Partial Credit Model (PCM)
...
wrightMap(fit$etapar, ppar$theta.table)
...
## The I[c]{style="color: OrangeRed;"}CCs

<mark style="background-color: #FFFF00">\@ Karl Noget at bemærke? Item 5?</mark>
...
Simulate 1000 data sets based on $\hat\beta$ and $\hat\theta$ Look at empirical distribution of $\chi^2$ for item of interest
